...
# Select the dataset
dataset_name = 'CIFAR10'  # Change to 'MNIST' or 'FashionMNIST' as needed
train_loader, test_loader, train_dataset, test_dataset = get_dataset(dataset_name, transform)
...
for i in range(4):
    ax = axes[i]
    imshow(ax,images[i])
    ax.set_title(f'Pred: {test_dataset.classes[predicted[i]]}')
    ax.axis('off')
...
# Load an image from file and make a prediction
def predict_image(image_path, model, transform, dataset_name):
